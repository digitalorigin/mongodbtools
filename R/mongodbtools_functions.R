...
mdb.findVars <- function(rmongodb, strCollection, strQuery, listVars, strFile, strDatabase = NULL) {
  ...
  if (mongodbtools:::use_log) multiplelines.message(paste0("[Query Input]: findVarsToCSV \n",strQuery))
  if (mongodbtools:::use_log) multiplelines.message(paste0("[Query Output]:\n File: ",strFile," \n"))
  timer = proc.time()
  results <- .jcall(rmongodb, "V", "findVarsToCSV", strCollection, strQuery, as.vector(listVars), strFile)
  timer = round(proc.time() - timer)
  if (mongodbtools:::use_log) message(paste0("[Query Execution Time: ",timer[3]," seconds.]\n"))
  invisible(NULL)
}
...
mdb.findVarsToJSON <- function(rmongodb, strCollection, strQuery, strFile, strDatabase = NULL) {
  if (!is.null(strDatabase)) mdb.useDatabase(rmongodb, strDatabase)
  if (mongodbtools:::use_log) multiplelines.message(paste0("[Query Time]: ",format(Sys.time(), "%Y%m%d_%H_%M_%S"),"\n"))
  if (mongodbtools:::use_log) multiplelines.message(paste0("[Query Input]: findVarsToJSON \n",strQuery))
  if (mongodbtools:::use_log) multiplelines.message(paste0("[Query Output]:\n File: ",strFile," \n"))
  timer = proc.time()
